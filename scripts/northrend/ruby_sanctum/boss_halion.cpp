...
//        m_creature->SetDisplayId(10045);
        ...
        m_pulsar_N = pInstance->GetSingleCreatureFromStorage(NPC_SHADOW_PULSAR_N);
        if (!m_pulsar_N )
        {
            float x,y;
        ...
        m_pulsar_S = pInstance->GetSingleCreatureFromStorage(NPC_SHADOW_PULSAR_S);
        if (!m_pulsar_S)
            ...
            for(Map::PlayerList::const_iterator i = pList.begin(); i != pList.end(); ++i)
            {
                if (Player* player = i->getSource())
                {
                    ...
                    if (player->isAlive())
                    {
                        ...
                        {
                            player->CastSpell(player, SPELL_TWILIGHT_CUTTER, true, 0, 0, m_pulsar_N->GetObjectGuid());
                        }
                    }
                }
            }
